...
export default authHandler(async (req: NextApiRequest, res: NextApiResponse<{ message: string; field?: string }>, user) => {
  if (req.method == 'POST') {
    ...
    if (newPassword != confirmPassword) return res.status(403).json({ message: 'Passwords do not match', field: 'confirmPassword' });
    if (!(await user.checkPassword(oldPassword))) return res.status(403).json({ message: 'Password is incorrect', field: 'oldPassword' });
