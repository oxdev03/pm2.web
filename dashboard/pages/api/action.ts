...
export default authHandler(async (req: NextApiRequest, res: NextApiResponse, user) => {
  ...
  try {
    switch (action) {
      case 'restart':
        await Process.updateOne({ _id: new ObjectId(id) }, { $inc: { restartCount: 1 } });
        break;
      case 'stop':
        await Process.updateOne({ _id: new ObjectId(id) }, { $inc: { toggleCount: 1 } });
        break;
      case 'delete':
        await Process.updateOne({ _id: new ObjectId(id) }, { $inc: { deleteCount: 1 } });
        break;
    }
  } catch (error) {
    console.error(error);
