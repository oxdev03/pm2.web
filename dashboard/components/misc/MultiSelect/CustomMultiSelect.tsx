...
export const CustomMultiSelect = factory<CustomMultiSelectFactory>((_props, ref) => {
  ...
  return (
    <>
      ...
      >
        <Combobox.DropdownTarget>
          <PillsInput
            ...
            disabled={disabled}
            radius={radius}
            rightSection={rightSection || clearButton || <Combobox.Chevron size={size} error={error} unstyled={unstyled} />}
        ...
        <Combobox.Dropdown hidden={(!searchable || !nothingFoundMessage || (hidePickedOptions && _searchValue.trim().length === 0)) && !options.length}>
          {withScrollArea ? (
            <ScrollArea.Autosize mah={maxDropdownHeight ?? 220} type="scroll" scrollbarSize="var(--_combobox-padding)" offsetScrollbars="y">
              {options}
            </ScrollArea.Autosize>
          ) : (
            options
      ...
      <input type="hidden" name={name} value={_value.join(hiddenInputValuesDivider)} form={form} disabled={disabled} {...hiddenInputProps} />
    </>
  );
});
