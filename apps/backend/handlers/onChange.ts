      ...
      $match: {
        $and: [
          { operationType: { $in: ["update"] } },
          {
            $or: [
              {
                "updateDescription.updatedFields.restartCount": {
                  $exists: true,
                },
              },
              {
                "updateDescription.updatedFields.toggleCount": {
                  $exists: true,
                },
              },
              {
                "updateDescription.updatedFields.deleteCount": {
                  $exists: true,
                },
              },
            ],
  ...
  changeStream.on("change", async (change) => {
    if (change.updateDescription?.updatedFields?.restartCount) {
      ...
      await processModel.updateOne(
        ...
        { $set: { restartCount: 0, toggleCount: 0, deleteCount: 0 } },
      );
    }
    if (change.updateDescription?.updatedFields?.toggleCount) {
      ...
      const process = processes.find(
        (item) => item.pm_id === change.fullDocument.pm_id,
      );
