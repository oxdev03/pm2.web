...
export function SelectedProvider({ children, servers }: { children: React.ReactNode; servers: IServer[] }) {
  ...
  const selectItem: SelectItem = (items, type) => {
    ...
    if (type == 'servers') {
      setSelectedItem({
        servers: items,
        processes: items.length ? selectedItem.processes.filter((process) => items.includes(allProcesses.find((item) => item._id == process)?.server || '')) : [],
      });
    } else if (type == 'processes') {
      setSelectedItem({
        servers: selectedItem.servers || [],
        processes: selectedItem.servers.length ? items.filter((process) => selectedItem.servers.includes(allProcesses.find((item) => item._id == process)?.server || '')) : items,
      });
    }
  };
