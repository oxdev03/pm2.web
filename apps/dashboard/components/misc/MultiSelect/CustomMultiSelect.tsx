...
export const CustomMultiSelect = factory<CustomMultiSelectFactory>(
  (_props, ref) => {
    ...
    const filteredData = data.filter((opt) =>
      opt.label.toLowerCase().includes(_searchValue.trim().toLowerCase()),
    );
        ...
        >
          ...
          <Combobox.Dropdown
            hidden={
              (!searchable ||
                !nothingFoundMessage ||
                (hidePickedOptions && _searchValue.trim().length === 0)) &&
              !options.length
            }
          >
            {withScrollArea ? (
            ...
            {!options.length && nothingFoundMessage && (
