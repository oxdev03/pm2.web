...
export default authHandler(
  async (
    ...
    if (!reqUser?.acl?.admin && !reqUser?.acl?.owner)
    ...
    switch (req.method) {
      case "DELETE":
        try {
          ...
          if (user.acl?.admin && !reqUser?.acl?.owner)
            return res.status(403).json({ message: "Unauthorized" });
      ...
      case "PATCH":
        try {
          ...
          if (user.acl?.owner && !reqUser?.acl?.owner)
            return res.status(403).json({ message: "Unauthorized" });
          if (!["owner", "admin", "custom", "none"].includes(permission))
          ...
          if (reqUser?.acl?.admin && permission === "owner")
            return res
              .status(403)
              .json({ message: "Admin cannot change permission to owner" });
            ...
            if (user.acl?.owner && permission !== "owner") {
              ...
              if (owners.length <= 1)
                return res
                  ...
                  .json({
                    message:
                      "Owner cannot be removed, Promote someone to an owner before",
                  });
            }
          }
          ...
          return res
            .status(200)
            .json({ message: "Updated permissions successfully" });
        } catch (error) {
          console.error(error);
          return res.status(500).json({ message: "Internal server error" });
        }
      ...
      case "POST":
        try {
          ...
          const users = await userModel.find({ _id: { $in: _users } });
          if (!users || !perms)
          ...
          perms = perms.filter((server) => {
            server.processes = server.processes.filter(
              (process) => !(process.perms === server.perms),
            );
