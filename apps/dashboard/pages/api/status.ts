  ...
  */
  const processes = await processModel.find(
    {
      ...(req?.body?.process?.length
        ? {
            _id: {
              $in: req.body.process?.map((x: string) => new ObjectId(x)),
            },
          }
        : {
            _id: { $exists: true },
  ...
  const logProcesses = processes
    .filter((process) => {
      ...
      if (user.acl.admin) return true;
      if (
        new Access(user.acl.servers).getPerms(process.server.toString(), process._id.toString()).has(PERMISSIONS.LOGS)
      )
        return true;
      return false;
    })
  ...
  const pipeline = [
    {
      $match: {
        ...(req?.body?.process?.length
          ? {
              _id: {
                $in: filteredProcesses?.map((x: string) => new ObjectId(x)),
              },
            }
          : {
              _id: { $exists: true },
        ...
        updatedAt: {
          $gt: new Date((req.body.timestamp || Date.now()) - 1000 * 60 * 4),
        },
      },
    },
    {
