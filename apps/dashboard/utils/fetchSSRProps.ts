...
export async function fetchServer(excludeDaemon?: boolean): Promise<IServer[]> {
  ...
  const _servers = await serverModel.find(
    {},
    {
      createdAt: 0,
    }
  ...
  for (let i = 0; i < servers.length; i++) {
    servers[i].processes = processes.filter((process) => process.server.toString() == servers[i]?._id?.toString() && (excludeDaemon ? process.name != 'pm2.web-daemon' : true));
  }
